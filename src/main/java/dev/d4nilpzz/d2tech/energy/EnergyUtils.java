...
public class EnergyUtils {
    public static void distributeNearly(BlockEntity blockEntity, ModEnergyStorage outputEnergyStorage, int transfer) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            IEnergyStorage energyStorage = level.getCapability(Capabilities.EnergyStorage.BLOCK, blockPos, direction.getOpposite());
            if (energyStorage != null) {
                int canSend = Math.min(outputEnergyStorage.getEnergyStored(), transfer);
                if (canSend <= 0) return;

                int amount = energyStorage.receiveEnergy(canSend, false);
